...
try:
    ...
    raise ImportError(
        "OAA attestation needs PyJWT and cryptography, which aren't installed "
        "by the core `guardrail-mcp` package. Install them with: "
        "pip install guardrail-mcp[oaa]"
    ) from exc
...
DEFAULT_TTL_SECONDS = 900  # 15 minutes - a decision token should not
# outlive the window in which the agent is expected to act on it. Pass
